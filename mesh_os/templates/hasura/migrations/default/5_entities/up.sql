...
-- Add helpful comment about using the sample data function
COMMENT ON FUNCTION insert_sample_data IS E'
Example usage:
1. First insert the sample data:
   SELECT insert_sample_data();

2. Then you can test with these GraphQL queries in Hasura Console:

# Query the test agent and its memories
query TestAgent {
  agents(where: {slug: {_eq: "test-agent"}}) {
    id
    name
    slug
    memories {
...
# Query entities and their linked memories
query TestEntities {
  entities(where: {ref_id: {_in: ["ai-concept", "programming"]}}) {
    id
    name
    type
    ref_id
...
# Test semantic search across memories and entities
query TestSearch {
  search_memories_and_entities(
    args: {
      query_embedding: [0.1] # Repeat 1536 times
      match_threshold: 0.7
      match_count: 10
    }
  ) {
...
# Test memory search filtered by entity
query TestMemorySearchByEntity {
  search_memories(
    args: {
      query_embedding: [0.1] # Repeat 1536 times
      match_threshold: 0.7
      match_count: 10
      filter_entity_id: "ENTITY_ID" # Replace with actual entity ID from previous query
    }
  ) {
    id
    content
...
3. SQL queries for reference:
- Search memories linked to an entity:
  SELECT * FROM search_memories(
    query_embedding := array_fill(0.1, ARRAY[1536])::vector(1536),
    match_threshold := 0.7,
    match_count := 10,
    filter_entity_id := (SELECT id FROM entities WHERE ref_id = ''ai-concept'')
  );

- Search both memories and entities:
  SELECT * FROM search_memories_and_entities(
    query_embedding := array_fill(0.1, ARRAY[1536])::vector(1536),
    match_threshold := 0.7,
    match_count := 10
  );
